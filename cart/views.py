...
def cart_show(request):
    ...
    conn = get_redis_connection('default')  # 拿到redis
...
def cart_del(req):
    ...
    if not all([books_id]):
        return JsonResponse({'res': 1, 'errmsg': '数据不完整'})
    books = Books.objects.get(books_id)
    if (books is None):
        return JsonResponse({'res': 2, 'errmsg': '返回为空'})
    conn = get_redis_connection("default")
    cart_key = 'cart_%d' % req.session.get('passport_id')
    conn.hget(cart_key, books_id)
    JsonResponse({'res': 3})
